...
function calcPolyMicroIndicators() {
  ...
  if (allCloses.length >= 15) {
    const rsiArr = calcRSI(allCloses, 14);
  ...
  windowTrades.forEach(t => { vwapNum += t.price * t.qty; vwapDen += t.qty; });
  ...
  if (allCloses.length >= 15) {
    ...
    const macdHistPrev = hist[hist.length - 2] || 0;
...
function updatePolyPanel() {
  ...
  if (dataStatusEl) {
    dataStatusEl.textContent = `Live • ${polyTrades.length} trades • ${polyCandles1m.filter(c => c.closed).length} closed candles`;
  }
  ...
  const signalEl = document.getElementById('polySignalText');
  if (signalEl) {
    const prefix = (score >= 65 || score <= 35) ? 'STRONG' : 'LEAN';
    if (label === 'NEUTRAL') {
      signalEl.textContent = 'NEUTRAL — No edge';
      signalEl.style.color = '#9ca3af';
    } else if (score >= 55) {
      signalEl.textContent = `${prefix === 'STRONG' ? 'STRONG UP' : 'LEAN UP'} — ${score}% confidence`;
      signalEl.style.color = '#10b981';
    } else {
      signalEl.textContent = `${prefix === 'STRONG' ? 'STRONG DOWN' : 'LEAN DOWN'} — ${100 - score}% confidence`;
