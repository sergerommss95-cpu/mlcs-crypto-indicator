...
const POLY_INDICATORS = ['MinuteDir', 'NetFlow', 'WindowTrend', 'TickMom', 'ROC', 'MACD', 'Spike', 'Whale', 'NetCandle'];
      ...
      if (polyMode) {
        ...
        if (ind) {
          ...
          const scored = [ind.minuteSignal, ind.netFlowSignal, ind.tickMomSignal,
            ...
            ind.spikeSignal, ind.whaleSignal, ind.netCandleSignal || 'neutral'];
          ...
          let confLevel = 'LOW';
          if (ind.minuteStreak >= 3 && (score >= 62 || score <= 38) && !ind.streakExhausting) {
            confLevel = consensus !== 'weak' ? 'HIGH' : 'MED';
          } else if (ind.minuteStreak >= 2 && (score >= 57 || score <= 43)) {
            confLevel = 'MED';
          } else if (score >= 58 || score <= 42) {
            confLevel = 'MED';
          } else if (agr >= 4 && (score >= 55 || score <= 45)) {
            confLevel = 'MED';
          }
...
function calcPolyMicroIndicators() {
  ...
  // v5.1 NEW: Net Candle Trend (catches sustained moves with micro-bounces)
  let netCandleRatio = 0;
  let netCandleSignal = 'neutral';
  let netCandleUp = 0;
  let netCandleDown = 0;
  if (closedCandles.length >= 5) {
    const nctLookback = Math.min(8, closedCandles.length);
    const nctRecent = closedCandles.slice(-nctLookback);
    for (const c of nctRecent) {
      if (c.close > c.open) netCandleUp++;
      else if (c.close < c.open) netCandleDown++;
    }
    const nctTotal = netCandleUp + netCandleDown;
    netCandleRatio = nctTotal > 0 ? (netCandleUp - netCandleDown) / nctTotal : 0;
    if (netCandleRatio > 0.25) netCandleSignal = 'bullish';
    else if (netCandleRatio < -0.25) netCandleSignal = 'bearish';
  }
...
function calcPolymarketSignal(ind) {
  // v5.1 regime-adaptive scoring with rebalanced weights
  let score = 50;

  // Get regime weight multipliers
  const rw = {
    trending: { minuteDir: 1.15, netFlow: 0.85, windowTrend: 1.20, tickMom: 1.0, roc: 1.0, macd: 1.0, spike: 0.5, whale: 1.0, netCandle: 1.30, scoreMult: 1.1 },
    ranging:  { minuteDir: 0.70, netFlow: 1.25, windowTrend: 0.80, tickMom: 1.0, roc: 1.0, macd: 1.2, spike: 1.5, whale: 1.2, netCandle: 0.70, scoreMult: 0.9 },
    volatile: { minuteDir: 0.90, netFlow: 1.0, windowTrend: 0.80, tickMom: 1.0, roc: 0.80, macd: 0.80, spike: 0.80, whale: 1.3, netCandle: 0.90, scoreMult: 0.85 },
  };
  const w = rw[ind.regime] || rw.ranging;

  // 1. MinuteDirection (±15 pts × regime) — v5.1: reduced from ±20
  if (ind.minuteStreak >= 3) {
    if (ind.streakExhausting) score += ind.minuteDir * 6 * w.minuteDir;
    else if (ind.streakDampened) score += ind.minuteDir * 10 * w.minuteDir;
    else score += ind.minuteDir * 15 * w.minuteDir;
  }
  if (ind.minuteStreak >= 4 && !ind.streakExhausting) {
    score += ind.minuteDir * 4 * w.minuteDir;
  }
  // 2. NetFlow (±12 pts × regime) — v5.1: increased from ±10
  score += Math.max(-12, Math.min(12, ind.netFlowVal * 60 * w.netFlow));
  // 3. Window Trend (±12 pts × regime) — v5.1: increased from ±7.5
  score += Math.max(-12, Math.min(12, ind.windowDev * 120 * w.windowTrend));
  // v5.1 NEW: Net Candle Trend (±8 pts × regime)
  score += Math.max(-8, Math.min(8, (ind.netCandleRatio || 0) * 16 * w.netCandle));
  ...
  // 7. Spike (±3 pts × regime)
  if (ind.spikeSignal === 'bullish') score += 3 * w.spike;
  else if (ind.spikeSignal === 'bearish') score -= 3 * w.spike;
  // 8. Whale (±2 pts × regime)
  if (ind.whaleSignal === 'bullish') score += 2 * w.whale;
  else if (ind.whaleSignal === 'bearish') score -= 2 * w.whale;

  // v5.1: Trend Momentum Override — when many indicators agree in trending regime
  const allSignals = [ind.minuteSignal, ind.netFlowSignal, ind.tickMomSignal,
    ind.roc > 0.005 ? 'bullish' : ind.roc < -0.005 ? 'bearish' : 'neutral',
    (ind.macdHistRising && ind.macdHist > 0) ? 'bullish' : (!ind.macdHistRising && ind.macdHist < 0) ? 'bearish' : 'neutral',
    ind.spikeSignal || 'neutral', ind.whaleSignal || 'neutral',
    ind.netCandleSignal || 'neutral'];
  const bullC = allSignals.filter(s => s === 'bullish' || s === 'up').length;
  const bearC = allSignals.filter(s => s === 'bearish' || s === 'down').length;
  if (ind.regime === 'trending' && Math.max(bullC, bearC) >= 5) {
    const consensusDir = bullC > bearC ? 1 : -1;
    score += consensusDir * 4;
  }
...
function updatePolyPanel() {
  ...
  if (dataStatusEl) {
    ...
    dataStatusEl.textContent = `v5.1 Live • ${polyTrades.length} trades • ${polyCandles1m.filter(c => c.closed).length} candles${streakInfo}${exhaustInfo}${regimeTag}${learnTag}`;
  }
  ...
  const signalEl = document.getElementById('polySignalText');
  if (signalEl) {
    // v5.1: Balanced confidence with relaxed gating
    const scored = [ind.minuteSignal, ind.netFlowSignal, ind.tickMomSignal,
      ind.roc > 0.005 ? 'bullish' : ind.roc < -0.005 ? 'bearish' : 'neutral',
      (ind.macdHistRising && ind.macdHist > 0) ? 'bullish' : (!ind.macdHistRising && ind.macdHist < 0) ? 'bearish' : 'neutral',
      ind.spikeSignal || 'neutral', ind.whaleSignal || 'neutral',
...
function polyRenderIndicators(ind) {
  ...
  const rows = [
    ...
    {
      name: 'Window Trend (18%)',
      value: `${ind.windowDev >= 0 ? '+' : ''}${(ind.windowDev * 100).toFixed(3)}%`,
      signal: ind.windowDev > 0.0001 ? 'up' : ind.windowDev < -0.0001 ? 'down' : 'neutral'
    },
    {
      name: 'Net Candle (12%)',
      value: `${ind.netCandleUp || 0}↑/${ind.netCandleDown || 0}↓`,
      signal: (ind.netCandleSignal === 'bullish') ? 'up' : (ind.netCandleSignal === 'bearish') ? 'down' : 'neutral'
    },
    {
      name: 'Tick Mom (10%)',
...
function polyUpdateLearnedWeights(prediction, actual, ind, confLevel) {
  ...
  const signals = {
    ...
    NetCandle:   ind.netCandleSignal || 'neutral',
  };
